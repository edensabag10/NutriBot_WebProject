...
export default function FoodSearch({ setScreen }) {
  ...
  const favorites = NutriBotClientService.getFavoriteFoods(user.id).map((food) => food.id);
  const foods = NutriBotClientService.getFoods().filter((food) => food.name.toLowerCase().includes(query.toLowerCase()));
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {foods.map((food) => (
            <div key={`${food.id}-${version}`} className="rounded-lg border border-slate-200 p-4">
              ...
              <div className="mt-3">
                <Button onClick={() => { NutriBotClientService.toggleFavoriteFood(user.id, food.id); setVersion(version + 1); }} variant={favorites.includes(food.id) ? 'secondary' : 'primary'}>
